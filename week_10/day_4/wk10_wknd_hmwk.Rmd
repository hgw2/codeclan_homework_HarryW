...
```{r}
mod56a <- lm(average_price ~ type + regions + quarter + year + log(total_volume) + regions:quarter, data = train)  
...
```{r}
model <- lm(glmulti_fit@formulas[[1]], data = train)

calc.relimp(model, type = "lmg", rela = TRUE)
```

```{r}
glmulti_best_trans <- lm(log(average_price)~ quarter+year+type+regions+log(total_volume)+if_else(x4225 > 0, log(x4225), 0)+if_else(x_large_bags > 0, log(x_large_bags), 0) +grouped_market + regions:quarter, data = train)

summary(glmulti_best)
```

```{r}
gli <- glance(glmulti_best) %>%  
...
predictions <- predict(glmulti_best, newdata = test)

mean((predictions - test$average_price)**2)

predictions <- predict(mod5d, newdata = test)
